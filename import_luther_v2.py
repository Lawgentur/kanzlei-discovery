...
def import_jobs():
    # Wir hängen die neuen Jobs einfach als Textzeilen an, um Header-Probleme zu vermeiden
    today = datetime.now().strftime("%Y-%m-%d")
    added_count = 0
    
    with open(MASTER_FILE, 'a', encoding='utf-8') as f:
        for job in LUTHER_JOBS:
            # Format: Kanzlei, Titel, Standort, URL, Datum
            line = f'"{job["Kanzlei"]}","{job["Titel"]}","{job["Standort"]}","{job["URL"]}","{today}"
'
            f.write(line)
            added_count += 1
